...
source("T:/Research folders/CCWTG/Analyses/Papers/Belongingness & Network Embeddedness/iso_source.R") #<- source code for isolates

#Look at the Global Environnment for errors! They will be datasets indicating where the isolate is.
#For example, a dataset called "F15_M_1" indicates that there is is an error in the Fall 2015 semester, Monday, survey 1.
#Run the source code to check if there are any nodes that should be missing. #If nothing gets outputted, this is good!

#No errors, checked on 3/20/2018 -Neil
...
w <- left_join(dplyr::rename(dplyr::as_data_frame(as_edgelist(g1)), Sender_Final_ID =V1 , Receiver_Final_ID = V2), temp_night, by = c("Sender_Final_ID","Receiver_Final_ID")) %>% 
                               filter(survnum == 1) #Match Weights
                           g1 <-  set_edge_attr(g1, "weight", value = w$sn2) #Add edge attribute

w <- left_join(dplyr::rename(dplyr::as_data_frame(as_edgelist(g2)), Sender_Final_ID =V1, Receiver_Final_ID = V2), temp_night, by = c("Sender_Final_ID","Receiver_Final_ID")) %>% 
                               filter(survnum == 2) #Match weights
                           g2 <-  set_edge_attr(g2, "weight", value = w$sn2) #Add edge attribute

w <- left_join(dplyr::rename(dplyr::as_data_frame(as_edgelist(g3)), Sender_Final_ID =V1, Receiver_Final_ID = V2), temp_night, by = c("Sender_Final_ID","Receiver_Final_ID")) %>% 
                               filter(survnum == 3) #Match weights
                           g3 <-  set_edge_attr(g3, "weight", value = w$sn2) #Add edge attribute

w <- left_join(dplyr::rename(dplyr::as_data_frame(as_edgelist(g4)), Sender_Final_ID =V1, Receiver_Final_ID = V2), temp_night, by = c("Sender_Final_ID","Receiver_Final_ID")) %>% 
                               filter(survnum == 4) #Match weights
                           g4 <-  set_edge_attr(g4, "weight", value = w$sn2) #Add edge attribute

w <- left_join(dplyr::rename(dplyr::as_data_frame(as_edgelist(g5)), Sender_Final_ID =V1, Receiver_Final_ID = V2), temp_night, by = c("Sender_Final_ID","Receiver_Final_ID")) %>% 
